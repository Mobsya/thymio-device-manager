...
#    endif
        ...
        boost::system::error_code read_ack(Device handle) {
            ...
            if(s == 8) {
                uint16_t msg_type = 0;
                std::memcpy(&msg_type, buffer + 4, sizeof(msg_type));
                msg_type = boost::endian::little_to_native(msg_type);
                if(msg_type == ASEBA_MESSAGE_BOOTLOADER_ACK) {
                    uint16_t err = 0;
                    std::memcpy(&err, buffer + 6, sizeof(err));
                    err = boost::endian::little_to_native(err);
        ...
        uint16_t get_node_id(Device handle) {
            ...
            if(s < 6)
                return 0;
            uint16_t msg_source = 0;
            std::memcpy(&msg_source, buffer + 2, sizeof(msg_source));
            msg_source = boost::endian::little_to_native(msg_source);
            return msg_source;
        }
