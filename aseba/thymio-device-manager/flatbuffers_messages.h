...
inline tagged_detached_flatbuffer create_save_response(uint32_t request_id, const aseba_node_registery::node_id& id,
    ...
    for(std::size_t i = 0; i < bytecode.size(); i++) {
        char interm[5] = {};
        const auto* format = i < 4 ? "%02x" : "%04x";
        std::snprintf(interm, sizeof(interm), format, bytecode[i]);
...
inline tagged_detached_flatbuffer create_compilation_result_response(uint32_t request_id,
    ...
    auto offset =
        mobsya::fb::CreateCompilationResultSuccess(fb, request_id, uint32_t(result.result->bytecode_size),
                                                   uint32_t(result.result->bytecode_total_size),
                                                   uint32_t(result.result->variables_size),
