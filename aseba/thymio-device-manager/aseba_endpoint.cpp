...
void aseba_endpoint::destroy() {
    ...
    boost::asio::post(m_io_context.get_executor(), [ctx = &m_io_context]() {
...
void aseba_endpoint::start() {
    ...
    schedule_send_ping(std::chrono::milliseconds(200));
...
void aseba_endpoint::schedule_send_ping(std::chrono::steady_clock::duration delay) {
    if(is_rebooting())
        return;
...
void aseba_endpoint::schedule_nodes_health_check(std::chrono::steady_clock::duration delay) {
