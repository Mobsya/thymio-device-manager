...
void aseba_node::schedule_execution_state_update() {
    ...
    m_status_timer.expires_after(std::chrono::seconds(1));
...
void aseba_node::request_device_info() {
    ...
    m_resend_timer.expires_after(std::chrono::seconds(1));
...
void aseba_node::schedule_variables_update(std::chrono::steady_clock::duration delay) {
    m_variables_timer.expires_after(delay);
    std::weak_ptr<aseba_node> ptr = shared_from_this();
    m_variables_timer.async_wait([ptr](boost::system::error_code ec) {
        ...
        that->schedule_variables_update(std::chrono::seconds(1));
    });
}
