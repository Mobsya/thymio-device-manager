...
void serial_acceptor_service::register_request(request& r) {
    m_active_timer.expires_after(std::chrono::milliseconds(1));
...
void serial_acceptor_service::on_active_timer(const boost::system::error_code& ec) {
    ...
    if(!m_requests.empty()) {
        m_active_timer.expires_after(std::chrono::milliseconds(500));  // :(
        m_active_timer.async_wait(boost::asio::bind_executor(
            m_strand, boost::bind(&serial_acceptor_service::on_active_timer, this, boost::placeholders::_1)));
    }
}
...
static std::string property_as_string(io_registry_entry_t ioRegistryEntry, const char* propertyKey) {
    ...
    const auto string_ref = CFStringRef(cfstr);
    if(const char* direct = CFStringGetCStringPtr(string_ref, kCFStringEncodingUTF8)) {
        std::string str(direct);
        str.erase(std::find_if(str.rbegin(), str.rend(), [](int ch) { return !std::isspace(ch); }).base(), str.end());
        return str;
    }

    const auto length = CFStringGetLength(string_ref);
    const auto max_size = CFStringGetMaximumSizeForEncoding(length, kCFStringEncodingUTF8);
    if(max_size < 0) {
        return {};
    }

    std::string str(static_cast<std::size_t>(max_size) + 1, '\0');
    if(!CFStringGetCString(string_ref, str.data(), static_cast<CFIndex>(str.size()), kCFStringEncodingUTF8)) {
        return {};
    }
